...
function makeReport(period: 'day' | 'month' | 'year', orders: AdminOrder[], expenses: AdminExpense[], events: AdminEvent[]) {
  const scopedOrders = filterByPeriod(orders, period)
  const scopedExpenses = filterByPeriod(expenses, period)
  const scopedEvents = filterByPeriod(events, period)
  const revenue = scopedOrders.filter((order) => order.status !== 'Cancelled').reduce((sum, order) => sum + orderRevenue(order), 0)
  const expenseTotal = scopedExpenses.reduce((sum, expense) => sum + expense.amount, 0)
  const fulfilled = scopedOrders.filter((order) => order.status === 'Fulfilled').length
  ...
  const rows = [
    ...
    ['Profit', String(revenue - expenseTotal)],
    ['Fulfilled Orders', String(fulfilled)],
    ['Cancelled Orders', String(cancelled)],
    ['Outstanding Revenue', String(scopedOrders.filter((order) => order.status === 'Outstanding').reduce((sum, order) => sum + orderRevenue(order), 0))],
    [],
...
function downloadReport(period: 'day' | 'month' | 'year', orders: AdminOrder[], expenses: AdminExpense[], events: AdminEvent[], type: 'financial' | 'orders' | 'expenses' | 'analysis') {
...
function DashboardPage({ orders, expenses, events, setActivePage }: { orders: AdminOrder[]; expenses: AdminExpense[]; events: AdminEvent[]; setActivePage: (page: AdminPage) => void }) {
  ...
  const todayRevenue = todayOrders.reduce((sum, order) => (order.status === 'Cancelled' ? sum : sum + orderRevenue(order)), 0)
  const outstandingRevenue = orders.filter((order) => order.status === 'Outstanding').reduce((sum, order) => sum + orderRevenue(order), 0)
  const cancelledRevenue = orders.filter((order) => order.status === 'Cancelled').reduce((sum, order) => sum + orderRevenue(order), 0)
  ...
  const funnel = [
    ...
    ['Delivered', eventCount('delivered')],
    ['Fulfilled', eventCount('fulfilled')],
  ] as const
  ...
  return (
    <div className="grid gap-6">
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-5">
        ...
        <StatCard label="Fulfilled Orders" value={orders.filter((order) => order.status === 'Fulfilled').length} tone="mint" />
        <StatCard label="Cancelled Orders" value={orders.filter((order) => order.status === 'Cancelled').length} tone="danger" />
        <StatCard label="Today's Profit" value={formatMoney(todayRevenue - todayExpenses)} tone="gold" />
      </div>

      <div className="grid gap-4 lg:grid-cols-5">
        <StatCard label="Today's Revenue" value={formatMoney(todayRevenue)} tone="mint" />
        <StatCard label="Today's Expenses" value={formatMoney(todayExpenses)} tone="danger" />
        <StatCard label="Today's Estimated Profit" value={formatMoney(todayRevenue - todayExpenses)} tone="gold" />
      ...
      <section className="rounded-[30px] border border-white/10 bg-white/[0.045] p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h3 className="mt-2 text-2xl font-black text-white">Todayâ€™s customer journey</h3>
          </div>
...
function OrderDetail({ order, onClose }: { order: AdminOrder; onClose: () => void }) {
  ...
  const addOrderExpense = () => {
    ...
    if (!parsed || !expensePurpose.trim()) return
  ...
  return (
    ...
    >
      ...
      <section className="mt-5 rounded-2xl border border-white/10 bg-white/[0.045] p-3">
        ...
        <input value={expenseAmount} onChange={(event) => setExpenseAmount(event.target.value.replace(/[^\d]/g, ''))} className="admin-input mt-3" placeholder="Amount (₦)" />
        ...
        <button type="button" onClick={addOrderExpense} className="mt-3 min-h-11 w-full rounded-full bg-gold-500 text-xs font-black uppercase tracking-[0.12em] text-ink-950">Add Expense</button>
      </section>
      <div className="mt-5 grid gap-2">
        <button type="button" onClick={() => navigator.clipboard?.writeText(copyText)} className="admin-action-button">
          <Copy className="size-4" /> Copy Order
        </button>
        {(['Contacted', 'Confirmed', 'Delivered', 'Fulfilled', 'Cancelled'] as AdminOrderStatus[]).map((status) => (
          <button type="button" className="admin-action-button" key={status} onClick={() => updateAdminOrderStatus(order.id, status)}>
            Mark {status}
          </button>
        ))}
        <button type="button" className="admin-action-button" onClick={() => updateAdminOrderStatus(order.id, 'Failed Delivery')}>
          Failed Delivery
        </button>
        <button type="button" className="admin-action-button text-red-300" onClick={() => deleteAdminOrder(order.id)}>
...
function AnalyticsPage({ orders, events, expenses }: { orders: AdminOrder[]; events: AdminEvent[]; expenses: AdminExpense[] }) {
  ...
  const revenue = orders.reduce((sum, order) => (order.status === 'Cancelled' ? sum : sum + orderRevenue(order)), 0)
  const expenseTotal = expenses.reduce((sum, expense) => sum + expense.amount, 0)
  const conversionRate = events.filter((event) => event.type === 'buy_click').length
    ? Math.round((orders.length / events.filter((event) => event.type === 'buy_click').length) * 100)
    : 0
  const topPackage =
    productPackages
      .map((item) => [item.title, orders.filter((order) => order.package.id === item.id).length] as const)
      .sort((a, b) => b[1] - a[1])[0]?.[0] ?? 'No package yet'
  const topState =
    nigerianStates
      .map((item) => [item, orders.filter((order) => order.customer.state === item).length] as const)
      .sort((a, b) => b[1] - a[1])[0]?.[0] ?? 'No state yet'
  const fulfilled = orders.filter((order) => order.status === 'Fulfilled').length
  const cancelled = orders.filter((order) => order.status === 'Cancelled').length
  const peakHour =
    Array.from({ length: 24 })
      .map((_, hour) => [hour, orders.filter((order) => new Date(order.createdAt).getHours() === hour).length] as const)
      .sort((a, b) => b[1] - a[1])[0]?.[0] ?? 0

  const analytics: Array<[string, string | number]> = [
    ['Visitors', events.filter((event) => event.type === 'visitor').length],
    ['Orders', orders.length],
    ['Revenue', formatMoney(revenue)],
    ['Expenses', formatMoney(expenseTotal)],
    ['Profit', formatMoney(revenue - expenseTotal)],
    ['Conversion Rate', `${conversionRate}%`],
    ['Top Selling Package', topPackage],
    ['Top Performing State', topState],
    ['Fulfillment Rate', `${orders.length ? Math.round((fulfilled / orders.length) * 100) : 0}%`],
    ['Cancellation Rate', `${orders.length ? Math.round((cancelled / orders.length) * 100) : 0}%`],
    ['Peak Ordering Hour', `${peakHour}:00`],
    ['Outstanding Revenue', formatMoney(orders.filter((order) => order.status === 'Outstanding').reduce((sum, order) => sum + orderRevenue(order), 0))],
  ]
  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="rounded-[30px] border border-white/10 bg-white/[0.045] p-5">
        ...
        <div className="mt-5 grid gap-3">
          {productPackages.map((item) => {
            const count = orders.filter((order) => order.package.id === item.id).length
...
function FinancePage({ orders, expenses }: { orders: AdminOrder[]; expenses: AdminExpense[] }) {
  ...
  const totalRevenue = orders.filter((order) => order.status !== 'Cancelled').reduce((sum, order) => sum + orderRevenue(order), 0)
  const recoveredRevenue = orders.filter((order) => order.status === 'Fulfilled').reduce((sum, order) => sum + orderRevenue(order), 0)
  const outstandingRevenue = orders.filter((order) => order.status === 'Outstanding').reduce((sum, order) => sum + orderRevenue(order), 0)
  const cancelledRevenue = orders.filter((order) => order.status === 'Cancelled').reduce((sum, order) => sum + orderRevenue(order), 0)
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <StatCard label="Total Revenue" value={formatMoney(totalRevenue)} tone="mint" />
      <StatCard label="Recovered Revenue" value={formatMoney(recoveredRevenue)} tone="mint" />
      <StatCard label="Outstanding Revenue" value={formatMoney(outstandingRevenue)} />
      <StatCard label="Cancelled Revenue" value={formatMoney(cancelledRevenue)} tone="danger" />
      <StatCard label="Total Expenses" value={formatMoney(totalExpenses)} tone="danger" />
      <StatCard label="Net Profit" value={formatMoney(netProfit)} tone="gold" />
      <StatCard label="ROI" value={`${roi}%`} tone="gold" />
      <StatCard label="Cash Flow" value={formatMoney(recoveredRevenue - totalExpenses)} />
    </div>
  )
}
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[380px_1fr]">
      <section className="rounded-[30px] border border-white/10 bg-white/[0.045] p-5">
        ...
        <input value={amount} onChange={(event) => setAmount(event.target.value.replace(/[^\d]/g, ''))} className="admin-input mt-5" placeholder="Amount (â‚¦)" />
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
        ...
        <div className="mt-5 grid gap-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button type="button" onClick={() => navigator.clipboard?.writeText(thankYouUrl)} className="admin-small-button shrink-0">Copy</button>
            </div>
          </div>
      ...
      <section className="rounded-[30px] border border-white/10 bg-white/[0.045] p-5 xl:col-span-2">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {(['day', 'month', 'year'] as const).flatMap((period) =>
...
export function AdminDashboard({ onClose }: AdminDashboardProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex bg-[#050505] text-white">
